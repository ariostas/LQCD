...
void pionSpecLight(){
    ...
    vector<vector<Double_t> > correlators092 = readData("pion.D-9199.DG2_1.DG2_1.SS");
    ...
    Double_t m1, e1;
    m1 = findMassBoot(correlators092, 5, 27, e1, 0.265, 2.06004e-02);

    masses.at(0).push_back(m1); masses.at(0).push_back(e1); masses.at(0).push_back(5); masses.at(0).push_back(27);
...
Double_t findMass(vector<vector<Double_t> > corr, Int_t nmin, Int_t nmax, Double_t massGuess, Double_t AGuess, Double_t *chi2){
    ...
    TF2 *fitFunc = new TF2("Fit function", func, massGuess-0.5, massGuess+0.5, AGuess-0.05, AGuess+0.05);
...
void scanFitRange(vector<vector<Double_t> > corr){
    ...
    for(Int_t x = 0; x < N; x++){
        m.at(x) = findMassBoot(corr, 15-x, 17+x, me.at(x), 0.265, 2.06004e-02, &(c2.at(x)), &(c2e.at(x)));
    }
