...
void pionSpecVarLight(){
    ...
    Double_t m1, e1;
    m1 = findMassBoot(gndcorr092, 4, 28, e1, 0.265, 6.86663e-01);
    masses.at(0).push_back(m1); masses.at(0).push_back(e1); masses.at(0).push_back(4); masses.at(0).push_back(28);

    vector<TString> names = {"Ground state", "1st excited state", "Remaining states"};

    graph(graphs092, names, "#Deltat", "Eigenvalues(#Deltat)", "pionVarCorrs092", 0, "log");

    graph(massgraphs092, names, "#Deltat", "M(#Deltat)", "pionVarMass092", &masses);
...
vector<vector<vector<Double_t> > > readData(TString mass){
    ...
    for(UInt_t x = 0; x < corrTypes.size(); x++){
        ...
        TString filename = "./VariationalLight/pion.D" + mass + "." + corrTypes.at(x);
...
Double_t findMass(vector<vector<Double_t> > corr, Int_t nmin, Int_t nmax, Double_t massGuess, Double_t AGuess, Double_t *chi2){
    ...
    TF2 *fitFunc = new TF2("Fit function", func, massGuess-0.2, massGuess+0.2, AGuess-0.2, AGuess+0.2);
...
void scanFitRange(vector<vector<Double_t> > corr){
    ...
    for(Int_t x = 0; x < N; x++){
        m.at(x) = findMassBoot(corr, 15-x, 17+x, me.at(x), 0.265, 6.86663e-01, &(c2.at(x)), &(c2e.at(x)));
    }
