#!/usr/bin/python3
n_sources = 5
n_sinks = 5
m = -1.0
seqsource_type = 'NUCL_D_UNPOL'
i = 0
t_sink = 16
s_size = 16
t_size = 64

def quark_smear(x):
    if(x == 0):
        return 'NONE'
    return 'GAUGE_INV_GAUSSIAN'

def link_smear(x):
    if(x == 0):
        return 'NONE'
    return 'APE_SMEAR'

def smear_factor(x):
    if(x == 0):
        return 0
    if(x == 1 or x == 2):
        return 1
    return 2

def smear_iter(x):
    if(x == 0):
        return 0
    elif(x == 1 or x == 2):
        return 5
    else:
        return 10

def disp_type(x):
    if(x == 2 or x == 4):
        return 'LAP-DERIV'
    return 'NONE'

def length(x):
    if(x == 2):
        return 2
    elif(x == 4):
        return 2
    return 0

f = open('bar3ptfnSEED.ini.xml', 'w')

f.write(
        '<?xml version="1.0"?>\n'+
        '<chroma>\n'+
        '<annotation>\n'+
        'Calculate hadron correlators\n'+
        '</annotation>\n'+
        '<Param> \n'+
        '  <InlineMeasurements>\n\n')

for x in range(n_sources):
    f.write('    <elem>\n'+
            '      <Name>MAKE_SOURCE</Name>\n'+
            '      <Frequency>1</Frequency>\n'+
            '      <Param>\n'+
            '        <version>6</version>\n'+
            '        <Source>\n'+
            '          <version>2</version>\n'+
            '          <SourceType>SHELL_SOURCE</SourceType>\n'+
            '          <j_decay>3</j_decay>\n'+
            '          <t_srce>0 0 0 0</t_srce>\n'+
            '          <SmearingParam>\n'+
            '              <wvf_kind>{0}</wvf_kind>\n'.format(quark_smear(x))+
            '              <wvf_param>{0}</wvf_param>\n'.format(smear_factor(x))+
            '              <wvfIntPar>{0}</wvfIntPar>\n'.format(smear_iter(x))+
            '              <no_smear_dir>3</no_smear_dir>\n'+
            '            </SmearingParam>\n'+
            '          <Displacement>\n'+
            '            <version>1</version>\n'+
            '            <DisplacementType>{0}</DisplacementType>\n'.format(disp_type(x))+
            '            <deriv_dir>0</deriv_dir>\n'+
            '            <deriv_length>{0}</deriv_length>\n'.format(length(x))+
            '          </Displacement>\n'+
            '          <LinkSmearing>\n'+
            '              <LinkSmearingType>{0}</LinkSmearingType>\n'.format(link_smear(x))+
            '              <link_smear_fact>{0}</link_smear_fact>\n'.format(smear_factor(x)+0.5)+
            '              <link_smear_num>{0}</link_smear_num>\n'.format(smear_iter(x))+
            '              <no_smear_dir>3</no_smear_dir>\n'+
            '            </LinkSmearing>\n'+
            '      </Source>\n'+
            '      </Param>\n'+
            '        <NamedObject>\n'+
            '          <gauge_id>default_gauge_field</gauge_id>\n'+
            '          <source_id>sh_{0}_source</source_id>\n'.format(x)+
            '        </NamedObject>\n'+
            '    </elem>\n\n')
    f.write('   <elem>\n'+
            '      <Name>PROPAGATOR</Name>\n'+
            '      <Frequency>1</Frequency>\n'+
            '      <Param>\n'+
            '        <version>10</version>\n'+
            '        <quarkSpinType>FULL</quarkSpinType>\n'+
            '        <obsvP>false</obsvP>\n'+
            '        <numRetries>1</numRetries>\n'+
            '        <FermionAction>\n'+
            '         <FermAct>WILSON</FermAct>\n'+
            '         <Mass>{0}</Mass>\n'.format(m)+
            '         <AnisoParam>\n'+
            '           <anisoP>true</anisoP>\n'+
            '           <t_dir>3</t_dir>\n'+
            '           <xi_0>2.0</xi_0>\n'+
            '           <nu>2.0</nu>\n'+
            '         </AnisoParam>\n'+
            '         <FermionBC>\n'+
            '           <FermBC>SIMPLE_FERMBC</FermBC>\n'+
            '           <boundary>1 1 1 -1</boundary>\n'+
            '         </FermionBC>\n'+
            '        </FermionAction>\n'+
            '    <InvertParam>\n'+
            '      <invType>CG_INVERTER</invType>\n'+
            '      <RsdCG>1.0e-10</RsdCG>\n'+
            '      <MaxCG>1000</MaxCG>\n'+
            '    </InvertParam>\n'+
            '      </Param>\n'+
            '       <NamedObject>\n'+
            '        <gauge_id>default_gauge_field</gauge_id>\n'+
            '        <source_id>sh_{0}_source</source_id>\n'.format(x)+
            '        <prop_id>sh_{0}_prop</prop_id>\n'.format(x)+
            '       </NamedObject>\n'+
            '    </elem>\n\n')
    f.write('   <elem>\n'+
            '      <Name>ERASE_NAMED_OBJECT</Name>\n'+
            '      <Frequency>1</Frequency>\n'+
            '      <NamedObject>\n'+
            '        <object_id>sh_{0}_source</object_id>\n'.format(x)+
            '      </NamedObject>\n'+
            '    </elem>\n\n')
    if(x == 2 or x == 4):
        f.write('    <elem>\n'+
                '      <Name>MAKE_SOURCE</Name>\n'+
                '      <Frequency>1</Frequency>\n'+
                '      <Param>\n'+
                '        <version>6</version>\n'+
                '        <Source>\n'+
                '          <version>2</version>\n'+
                '          <SourceType>SHELL_SOURCE</SourceType>\n'+
                '          <j_decay>3</j_decay>\n'+
                '          <t_srce>0 0 0 0</t_srce>\n'+
                '          <SmearingParam>\n'+
                '              <wvf_kind>{0}</wvf_kind>\n'.format(quark_smear(x))+
                '              <wvf_param>{0}</wvf_param>\n'.format(smear_factor(x))+
                '              <wvfIntPar>{0}</wvfIntPar>\n'.format(smear_iter(x))+
                '              <no_smear_dir>3</no_smear_dir>\n'+
                '            </SmearingParam>\n'+
                '          <Displacement>\n'+
                '            <version>1</version>\n'+
                '            <DisplacementType>{0}</DisplacementType>\n'.format('NONE')+
                '            <deriv_dir>0</deriv_dir>\n'+
                '            <deriv_length>{0}</deriv_length>\n'.format(length(x))+
                '          </Displacement>\n'+
                '          <LinkSmearing>\n'+
                '              <LinkSmearingType>{0}</LinkSmearingType>\n'.format(link_smear(x))+
                '              <link_smear_fact>{0}</link_smear_fact>\n'.format(smear_factor(x)+0.5)+
                '              <link_smear_num>{0}</link_smear_num>\n'.format(smear_iter(x))+
                '              <no_smear_dir>3</no_smear_dir>\n'+
                '            </LinkSmearing>\n'+
                '      </Source>\n'+
                '      </Param>\n'+
                '        <NamedObject>\n'+
                '          <gauge_id>default_gauge_field</gauge_id>\n'+
                '          <source_id>sh_{0}_source_nodisp</source_id>\n'.format(x)+
                '        </NamedObject>\n'+
                '    </elem>\n\n')
        f.write('   <elem>\n'+
                '      <Name>PROPAGATOR</Name>\n'+
                '      <Frequency>1</Frequency>\n'+
                '      <Param>\n'+
                '        <version>10</version>\n'+
                '        <quarkSpinType>FULL</quarkSpinType>\n'+
                '        <obsvP>false</obsvP>\n'+
                '        <numRetries>1</numRetries>\n'+
                '        <FermionAction>\n'+
                '         <FermAct>WILSON</FermAct>\n'+
                '         <Mass>{0}</Mass>\n'.format(m)+
                '         <AnisoParam>\n'+
                '           <anisoP>true</anisoP>\n'+
                '           <t_dir>3</t_dir>\n'+
                '           <xi_0>2.0</xi_0>\n'+
                '           <nu>2.0</nu>\n'+
                '         </AnisoParam>\n'+
                '         <FermionBC>\n'+
                '           <FermBC>SIMPLE_FERMBC</FermBC>\n'+
                '           <boundary>1 1 1 -1</boundary>\n'+
                '         </FermionBC>\n'+
                '        </FermionAction>\n'+
                '    <InvertParam>\n'+
                '      <invType>CG_INVERTER</invType>\n'+
                '      <RsdCG>1.0e-10</RsdCG>\n'+
                '      <MaxCG>1000</MaxCG>\n'+
                '    </InvertParam>\n'+
                '      </Param>\n'+
                '       <NamedObject>\n'+
                '        <gauge_id>default_gauge_field</gauge_id>\n'+
                '        <source_id>sh_{0}_source_nodisp</source_id>\n'.format(x)+
                '        <prop_id>sh_{0}_prop_nodisp</prop_id>\n'.format(x)+
                '       </NamedObject>\n'+
                '    </elem>\n\n')
        f.write('   <elem>\n'+
                '      <Name>ERASE_NAMED_OBJECT</Name>\n'+
                '      <Frequency>1</Frequency>\n'+
                '      <NamedObject>\n'+
                '        <object_id>sh_{0}_source_nodisp</object_id>\n'.format(x)+
                '      </NamedObject>\n'+
                '    </elem>\n\n')
    for y in range(n_sinks):
        if(x != 2 and x != 4 and y != 2 and y != 4):
            f.write('    <elem>\n'+
                    '      <Name>SINK_SMEAR</Name>\n'+
                    '      <Frequency>1</Frequency>\n'+
                    '      <Param>\n'+
                    '        <version>5</version>\n'+
                    '        <Sink>\n'+
                    '          <version>2</version>\n'+
                    '          <SinkType>SHELL_SINK</SinkType>\n'+
                    '          <j_decay>3</j_decay>\n'+
                    '          <Displacement>\n'+
                    '            <version>1</version>\n'+
                    '            <DisplacementType>{0}</DisplacementType>\n'.format(disp_type(y))+
                    '            <deriv_dir>0</deriv_dir>\n'+
                    '            <deriv_length>{0}</deriv_length>\n'.format(length(y))+
                    '          </Displacement>\n'+
                    '        <SmearingParam>\n'+
                    '            <wvf_kind>{0}</wvf_kind>\n'.format(quark_smear(y))+
                    '            <wvf_param>{0}</wvf_param>\n'.format(smear_factor(y))+
                    '            <wvfIntPar>{0}</wvfIntPar>\n'.format(smear_iter(y))+
                    '            <no_smear_dir>3</no_smear_dir>\n'+
                    '          </SmearingParam>\n'+
                    '          <LinkSmearing>\n'+
                    '            <LinkSmearingType>{0}</LinkSmearingType>\n'.format(link_smear(y))+
                    '            <link_smear_fact>{0}</link_smear_fact>\n'.format(smear_factor(y)+0.5)+
                    '            <link_smear_num>{0}</link_smear_num>\n'.format(smear_iter(y))+
                    '            <no_smear_dir>3</no_smear_dir>\n'+
                    '          </LinkSmearing>\n'+
                    '        </Sink>\n'+
                    '      </Param>\n'+
                    '      <NamedObject>\n'+
                    '        <gauge_id>default_gauge_field</gauge_id>\n'+
                    '        <prop_id>sh_{0}_prop</prop_id>\n'.format(x)+
                    '        <smeared_prop_id>sh_{0}_sh_{1}_sink</smeared_prop_id>\n'.format(x,y)+
                    '      </NamedObject>\n'+
                    '    </elem>\n\n')
        if((x != 2 and x != 4) and (y == 2 or y == 4)):
            f.write('    <elem>\n'+
                    '      <Name>SINK_SMEAR</Name>\n'+
                    '      <Frequency>1</Frequency>\n'+
                    '      <Param>\n'+
                    '        <version>5</version>\n'+
                    '        <Sink>\n'+
                    '          <version>2</version>\n'+
                    '          <SinkType>SHELL_SINK</SinkType>\n'+
                    '          <j_decay>3</j_decay>\n'+
                    '          <Displacement>\n'+
                    '            <version>1</version>\n'+
                    '            <DisplacementType>{0}</DisplacementType>\n'.format(disp_type(y))+
                    '            <deriv_dir>0</deriv_dir>\n'+
                    '            <deriv_length>{0}</deriv_length>\n'.format(length(y))+
                    '          </Displacement>\n'+
                    '        <SmearingParam>\n'+
                    '            <wvf_kind>{0}</wvf_kind>\n'.format(quark_smear(y))+
                    '            <wvf_param>{0}</wvf_param>\n'.format(smear_factor(y))+
                    '            <wvfIntPar>{0}</wvfIntPar>\n'.format(smear_iter(y))+
                    '            <no_smear_dir>3</no_smear_dir>\n'+
                    '          </SmearingParam>\n'+
                    '          <LinkSmearing>\n'+
                    '            <LinkSmearingType>{0}</LinkSmearingType>\n'.format(link_smear(y))+
                    '            <link_smear_fact>{0}</link_smear_fact>\n'.format(smear_factor(y)+0.5)+
                    '            <link_smear_num>{0}</link_smear_num>\n'.format(smear_iter(y))+
                    '            <no_smear_dir>3</no_smear_dir>\n'+
                    '          </LinkSmearing>\n'+
                    '        </Sink>\n'+
                    '      </Param>\n'+
                    '      <NamedObject>\n'+
                    '        <gauge_id>default_gauge_field</gauge_id>\n'+
                    '        <prop_id>sh_{0}_prop</prop_id>\n'.format(x)+
                    '        <smeared_prop_id>sh_{0}_sh_{1}_sink</smeared_prop_id>\n'.format(x,y)+
                    '      </NamedObject>\n'+
                    '    </elem>\n\n')
            f.write('    <elem>\n'+
                    '      <Name>SINK_SMEAR</Name>\n'+
                    '      <Frequency>1</Frequency>\n'+
                    '      <Param>\n'+
                    '        <version>5</version>\n'+
                    '        <Sink>\n'+
                    '          <version>2</version>\n'+
                    '          <SinkType>SHELL_SINK</SinkType>\n'+
                    '          <j_decay>3</j_decay>\n'+
                    '          <Displacement>\n'+
                    '            <version>1</version>\n'+
                    '            <DisplacementType>{0}</DisplacementType>\n'.format('NONE')+
                    '            <deriv_dir>0</deriv_dir>\n'+
                    '            <deriv_length>{0}</deriv_length>\n'.format(length(y))+
                    '          </Displacement>\n'+
                    '        <SmearingParam>\n'+
                    '            <wvf_kind>{0}</wvf_kind>\n'.format(quark_smear(y))+
                    '            <wvf_param>{0}</wvf_param>\n'.format(smear_factor(y))+
                    '            <wvfIntPar>{0}</wvfIntPar>\n'.format(smear_iter(y))+
                    '            <no_smear_dir>3</no_smear_dir>\n'+
                    '          </SmearingParam>\n'+
                    '          <LinkSmearing>\n'+
                    '            <LinkSmearingType>{0}</LinkSmearingType>\n'.format(link_smear(y))+
                    '            <link_smear_fact>{0}</link_smear_fact>\n'.format(smear_factor(y)+0.5)+
                    '            <link_smear_num>{0}</link_smear_num>\n'.format(smear_iter(y))+
                    '            <no_smear_dir>3</no_smear_dir>\n'+
                    '          </LinkSmearing>\n'+
                    '        </Sink>\n'+
                    '      </Param>\n'+
                    '      <NamedObject>\n'+
                    '        <gauge_id>default_gauge_field</gauge_id>\n'+
                    '        <prop_id>sh_{0}_prop</prop_id>\n'.format(x)+
                    '        <smeared_prop_id>sh_{0}_sh_{1}_sink_nodisp</smeared_prop_id>\n'.format(x,y)+
                    '      </NamedObject>\n'+
                    '    </elem>\n\n')
        if((x == 2 or x == 4) and (y != 2 and y != 4)):
            f.write('    <elem>\n'+
                    '      <Name>SINK_SMEAR</Name>\n'+
                    '      <Frequency>1</Frequency>\n'+
                    '      <Param>\n'+
                    '        <version>5</version>\n'+
                    '        <Sink>\n'+
                    '          <version>2</version>\n'+
                    '          <SinkType>SHELL_SINK</SinkType>\n'+
                    '          <j_decay>3</j_decay>\n'+
                    '          <Displacement>\n'+
                    '            <version>1</version>\n'+
                    '            <DisplacementType>{0}</DisplacementType>\n'.format(disp_type(y))+
                    '            <deriv_dir>0</deriv_dir>\n'+
                    '            <deriv_length>{0}</deriv_length>\n'.format(length(y))+
                    '          </Displacement>\n'+
                    '        <SmearingParam>\n'+
                    '            <wvf_kind>{0}</wvf_kind>\n'.format(quark_smear(y))+
                    '            <wvf_param>{0}</wvf_param>\n'.format(smear_factor(y))+
                    '            <wvfIntPar>{0}</wvfIntPar>\n'.format(smear_iter(y))+
                    '            <no_smear_dir>3</no_smear_dir>\n'+
                    '          </SmearingParam>\n'+
                    '          <LinkSmearing>\n'+
                    '            <LinkSmearingType>{0}</LinkSmearingType>\n'.format(link_smear(y))+
                    '            <link_smear_fact>{0}</link_smear_fact>\n'.format(smear_factor(y)+0.5)+
                    '            <link_smear_num>{0}</link_smear_num>\n'.format(smear_iter(y))+
                    '            <no_smear_dir>3</no_smear_dir>\n'+
                    '          </LinkSmearing>\n'+
                    '        </Sink>\n'+
                    '      </Param>\n'+
                    '      <NamedObject>\n'+
                    '        <gauge_id>default_gauge_field</gauge_id>\n'+
                    '        <prop_id>sh_{0}_prop</prop_id>\n'.format(x)+
                    '        <smeared_prop_id>sh_{0}_sh_{1}_sink</smeared_prop_id>\n'.format(x,y)+
                    '      </NamedObject>\n'+
                    '    </elem>\n\n')
            f.write('    <elem>\n'+
                    '      <Name>SINK_SMEAR</Name>\n'+
                    '      <Frequency>1</Frequency>\n'+
                    '      <Param>\n'+
                    '        <version>5</version>\n'+
                    '        <Sink>\n'+
                    '          <version>2</version>\n'+
                    '          <SinkType>SHELL_SINK</SinkType>\n'+
                    '          <j_decay>3</j_decay>\n'+
                    '          <Displacement>\n'+
                    '            <version>1</version>\n'+
                    '            <DisplacementType>{0}</DisplacementType>\n'.format(disp_type(y))+
                    '            <deriv_dir>0</deriv_dir>\n'+
                    '            <deriv_length>{0}</deriv_length>\n'.format(length(y))+
                    '          </Displacement>\n'+
                    '        <SmearingParam>\n'+
                    '            <wvf_kind>{0}</wvf_kind>\n'.format(quark_smear(y))+
                    '            <wvf_param>{0}</wvf_param>\n'.format(smear_factor(y))+
                    '            <wvfIntPar>{0}</wvfIntPar>\n'.format(smear_iter(y))+
                    '            <no_smear_dir>3</no_smear_dir>\n'+
                    '          </SmearingParam>\n'+
                    '          <LinkSmearing>\n'+
                    '            <LinkSmearingType>{0}</LinkSmearingType>\n'.format(link_smear(y))+
                    '            <link_smear_fact>{0}</link_smear_fact>\n'.format(smear_factor(y)+0.5)+
                    '            <link_smear_num>{0}</link_smear_num>\n'.format(smear_iter(y))+
                    '            <no_smear_dir>3</no_smear_dir>\n'+
                    '          </LinkSmearing>\n'+
                    '        </Sink>\n'+
                    '      </Param>\n'+
                    '      <NamedObject>\n'+
                    '        <gauge_id>default_gauge_field</gauge_id>\n'+
                    '        <prop_id>sh_{0}_prop_nodisp</prop_id>\n'.format(x)+
                    '        <smeared_prop_id>sh_{0}_sh_{1}_sink_nodisp</smeared_prop_id>\n'.format(x,y)+
                    '      </NamedObject>\n'+
                    '    </elem>\n\n')
        if((x == 2 or x == 4) and (y == 2 or y == 4)):
            f.write('    <elem>\n'+
                    '      <Name>SINK_SMEAR</Name>\n'+
                    '      <Frequency>1</Frequency>\n'+
                    '      <Param>\n'+
                    '        <version>5</version>\n'+
                    '        <Sink>\n'+
                    '          <version>2</version>\n'+
                    '          <SinkType>SHELL_SINK</SinkType>\n'+
                    '          <j_decay>3</j_decay>\n'+
                    '          <Displacement>\n'+
                    '            <version>1</version>\n'+
                    '            <DisplacementType>{0}</DisplacementType>\n'.format(disp_type(y))+
                    '            <deriv_dir>0</deriv_dir>\n'+
                    '            <deriv_length>{0}</deriv_length>\n'.format(length(y))+
                    '          </Displacement>\n'+
                    '        <SmearingParam>\n'+
                    '            <wvf_kind>{0}</wvf_kind>\n'.format(quark_smear(y))+
                    '            <wvf_param>{0}</wvf_param>\n'.format(smear_factor(y))+
                    '            <wvfIntPar>{0}</wvfIntPar>\n'.format(smear_iter(y))+
                    '            <no_smear_dir>3</no_smear_dir>\n'+
                    '          </SmearingParam>\n'+
                    '          <LinkSmearing>\n'+
                    '            <LinkSmearingType>{0}</LinkSmearingType>\n'.format(link_smear(y))+
                    '            <link_smear_fact>{0}</link_smear_fact>\n'.format(smear_factor(y)+0.5)+
                    '            <link_smear_num>{0}</link_smear_num>\n'.format(smear_iter(y))+
                    '            <no_smear_dir>3</no_smear_dir>\n'+
                    '          </LinkSmearing>\n'+
                    '        </Sink>\n'+
                    '      </Param>\n'+
                    '      <NamedObject>\n'+
                    '        <gauge_id>default_gauge_field</gauge_id>\n'+
                    '        <prop_id>sh_{0}_prop</prop_id>\n'.format(x)+
                    '        <smeared_prop_id>sh_{0}_sh_{1}_sink</smeared_prop_id>\n'.format(x,y)+
                    '      </NamedObject>\n'+
                    '    </elem>\n\n')
            f.write('    <elem>\n'+
                    '      <Name>SINK_SMEAR</Name>\n'+
                    '      <Frequency>1</Frequency>\n'+
                    '      <Param>\n'+
                    '        <version>5</version>\n'+
                    '        <Sink>\n'+
                    '          <version>2</version>\n'+
                    '          <SinkType>SHELL_SINK</SinkType>\n'+
                    '          <j_decay>3</j_decay>\n'+
                    '          <Displacement>\n'+
                    '            <version>1</version>\n'+
                    '            <DisplacementType>{0}</DisplacementType>\n'.format('NONE')+
                    '            <deriv_dir>0</deriv_dir>\n'+
                    '            <deriv_length>{0}</deriv_length>\n'.format(length(y))+
                    '          </Displacement>\n'+
                    '        <SmearingParam>\n'+
                    '            <wvf_kind>{0}</wvf_kind>\n'.format(quark_smear(y))+
                    '            <wvf_param>{0}</wvf_param>\n'.format(smear_factor(y))+
                    '            <wvfIntPar>{0}</wvfIntPar>\n'.format(smear_iter(y))+
                    '            <no_smear_dir>3</no_smear_dir>\n'+
                    '          </SmearingParam>\n'+
                    '          <LinkSmearing>\n'+
                    '            <LinkSmearingType>{0}</LinkSmearingType>\n'.format(link_smear(y))+
                    '            <link_smear_fact>{0}</link_smear_fact>\n'.format(smear_factor(y)+0.5)+
                    '            <link_smear_num>{0}</link_smear_num>\n'.format(smear_iter(y))+
                    '            <no_smear_dir>3</no_smear_dir>\n'+
                    '          </LinkSmearing>\n'+
                    '        </Sink>\n'+
                    '      </Param>\n'+
                    '      <NamedObject>\n'+
                    '        <gauge_id>default_gauge_field</gauge_id>\n'+
                    '        <prop_id>sh_{0}_prop_nodisp</prop_id>\n'.format(x)+
                    '        <smeared_prop_id>sh_{0}_sh_{1}_sink_nodisp</smeared_prop_id>\n'.format(x,y)+
                    '      </NamedObject>\n'+
                    '    </elem>\n\n')
        if(x != 2 and x != 4 and y != 2 and y != 4):
            f.write('    <elem>\n'+
                    '      <Name>HADRON_SPECTRUM</Name>\n'+
                    '      <Frequency>1</Frequency>\n'+
                    '      <Param>\n'+
                    '        <version>1</version>\n'+
                    '        <MesonP>true</MesonP>\n'+
                    '        <CurrentP>true</CurrentP>\n'+
                    '        <BaryonP>true</BaryonP>\n'+
                    '        <time_rev>false</time_rev>\n'+
                    '        <mom2_max>1</mom2_max>\n'+
                    '        <avg_equiv_mom>false</avg_equiv_mom>\n'+
                    '       </Param>\n'+
                    '      <NamedObject>\n'+
                    '        <gauge_id>default_gauge_field</gauge_id>\n'+
                    '        <sink_pairs>\n'+
                    '          <elem>\n'+
                    '            <first_id>sh_{0}_sh_{1}_sink</first_id>\n'.format(x,y)+
                    '            <second_id>sh_{0}_sh_{1}_sink</second_id>\n'.format(x,y)+
                    '          </elem>\n'+
                    '        </sink_pairs>\n'+
                    '      </NamedObject>\n'+
                    '      <xml_file>hadspec_sh_{0}_sh_{1}_CONFIGNAME_t{2}.dat.xml</xml_file>\n'.format(x,y,t_sink)+
                    '    </elem>\n\n')
            f.write('   <elem>\n'+
                    '      <Name>ERASE_NAMED_OBJECT</Name>\n'+
                    '      <Frequency>1</Frequency>\n'+
                    '      <NamedObject>\n'+
                    '        <object_id>sh_{0}_sh_{1}_sink</object_id>\n'.format(x,y)+
                    '      </NamedObject>\n'+
                    '    </elem>\n\n')
        else:
            f.write('    <elem>\n'+
                    '      <Name>HADRON_SPECTRUM</Name>\n'+
                    '      <Frequency>1</Frequency>\n'+
                    '      <Param>\n'+
                    '        <version>1</version>\n'+
                    '        <MesonP>true</MesonP>\n'+
                    '        <CurrentP>true</CurrentP>\n'+
                    '        <BaryonP>true</BaryonP>\n'+
                    '        <time_rev>false</time_rev>\n'+
                    '        <mom2_max>1</mom2_max>\n'+
                    '        <avg_equiv_mom>false</avg_equiv_mom>\n'+
                    '       </Param>\n'+
                    '      <NamedObject>\n'+
                    '        <gauge_id>default_gauge_field</gauge_id>\n'+
                    '        <sink_pairs>\n'+
                    '          <elem>\n'+
                    '            <first_id>sh_{0}_sh_{1}_sink_nodisp</first_id>\n'.format(x,y)+
                    '            <second_id>sh_{0}_sh_{1}_sink</second_id>\n'.format(x,y)+
                    '          </elem>\n'+
                    '        </sink_pairs>\n'+
                    '      </NamedObject>\n'+
                    '      <xml_file>hadspec_sh_{0}_sh_{1}_CONFIGNAME_t{2}.dat.xml</xml_file>\n'.format(x,y,t_sink)+
                    '    </elem>\n\n')
            f.write('   <elem>\n'+
                    '      <Name>ERASE_NAMED_OBJECT</Name>\n'+
                    '      <Frequency>1</Frequency>\n'+
                    '      <NamedObject>\n'+
                    '        <object_id>sh_{0}_sh_{1}_sink</object_id>\n'.format(x,y)+
                    '      </NamedObject>\n'+
                    '    </elem>\n\n')
            f.write('   <elem>\n'+
                    '      <Name>ERASE_NAMED_OBJECT</Name>\n'+
                    '      <Frequency>1</Frequency>\n'+
                    '      <NamedObject>\n'+
                    '        <object_id>sh_{0}_sh_{1}_sink_nodisp</object_id>\n'.format(x,y)+
                    '      </NamedObject>\n'+
                    '    </elem>\n\n')
        f.write(
                '    <elem>\n'+
                '      <Name>SEQSOURCE</Name>\n'+
                '      <Frequency>1</Frequency>\n'+
                '      <Param>\n'+
                '        <version>1</version>\n'+
                '        <seq_src>{0}</seq_src>\n'.format(seqsource_type)+
                '        <t_sink>{0}</t_sink>\n'.format(t_sink)+
                '        <sink_mom>0 0 0</sink_mom>\n'+
                '      </Param>\n'+
                '      <PropSink>\n'+
                '        <version>5</version>\n'+
                '        <Sink>\n'+
                '          <version>2</version>\n'+
                '          <SinkType>SHELL_SINK</SinkType>\n'+
                '          <j_decay>3</j_decay>\n'+
                '          <Displacement>\n'+
                '            <version>1</version>\n'+
                '            <DisplacementType>{0}</DisplacementType>\n'.format('NONE')+
                '            <deriv_dir>0</deriv_dir>\n'+
                '            <deriv_length>{0}</deriv_length>\n'.format(length(y))+
                '          </Displacement>\n'+
                '        <SmearingParam>\n'+
                '            <wvf_kind>{0}</wvf_kind>\n'.format(quark_smear(y))+
                '            <wvf_param>{0}</wvf_param>\n'.format(smear_factor(y))+
                '            <wvfIntPar>{0}</wvfIntPar>\n'.format(smear_iter(y))+
                '            <no_smear_dir>3</no_smear_dir>\n'+
                '          </SmearingParam>\n'+
                '          <LinkSmearing>\n'+
                '            <LinkSmearingType>{0}</LinkSmearingType>\n'.format(link_smear(y))+
                '            <link_smear_fact>{0}</link_smear_fact>\n'.format(smear_factor(y)+0.5)+
                '            <link_smear_num>{0}</link_smear_num>\n'.format(smear_iter(y))+
                '            <no_smear_dir>3</no_smear_dir>\n'+
                '          </LinkSmearing>\n'+
                '        <Source>\n'+
                '          <version>2</version>\n'+
                '          <SourceType>SHELL_SOURCE</SourceType>\n'+
                '          <j_decay>3</j_decay>\n'+
                '          <t_srce>0 0 0 0</t_srce>\n'+
                '          <SmearingParam>\n'+
                '              <wvf_kind>{0}</wvf_kind>\n'.format(quark_smear(y))+
                '              <wvf_param>{0}</wvf_param>\n'.format(smear_factor(y))+
                '              <wvfIntPar>{0}</wvfIntPar>\n'.format(smear_iter(y))+
                '              <no_smear_dir>3</no_smear_dir>\n'+
                '            </SmearingParam>\n'+
                '          <Displacement>\n'+
                '            <version>1</version>\n'+
                '            <DisplacementType>{0}</DisplacementType>\n'.format(disp_type(y))+
                '            <deriv_dir>0</deriv_dir>\n'+
                '            <deriv_length>{0}</deriv_length>\n'.format(length(y))+
                '          </Displacement>\n'+
                '          <LinkSmearing>\n'+
                '              <LinkSmearingType>{0}</LinkSmearingType>\n'.format(link_smear(y))+
                '              <link_smear_fact>{0}</link_smear_fact>\n'.format(smear_factor(y)+0.5)+
                '              <link_smear_num>{0}</link_smear_num>\n'.format(smear_iter(y))+
                '              <no_smear_dir>3</no_smear_dir>\n'+
                '            </LinkSmearing>\n'+
                '      </Source>\n'+
                '        </Sink>\n'+
                '      </PropSink>\n'+
                '      <NamedObject>\n'+
                '        <gauge_id>default_gauge_field</gauge_id>\n'+
                '        <prop_ids>\n')
        if(x == 2 or x == 4):
            f.write('          <elem>sh_{0}_prop_nodisp</elem>\n'.format(x)+
                    '        </prop_ids>\n'+
                    '        <seqsource_id>sh_{0}_sh_{1}_seqsource</seqsource_id>\n'.format(x,y)+
                    '      </NamedObject>\n'+
                    '    </elem>\n\n')
        else:
            f.write('          <elem>sh_{0}_prop</elem>\n'.format(x)+
                    '        </prop_ids>\n'+
                    '        <seqsource_id>sh_{0}_sh_{1}_seqsource</seqsource_id>\n'.format(x,y)+
                    '      </NamedObject>\n'+
                    '    </elem>\n\n')
        f.write('   <elem>\n'+
                '      <Name>PROPAGATOR</Name>\n'+
                '      <Frequency>1</Frequency>\n'+
                '      <Param>\n'+
                '        <version>10</version>\n'+
                '        <quarkSpinType>FULL</quarkSpinType>\n'+
                '        <obsvP>false</obsvP>\n'+
                '        <numRetries>1</numRetries>\n'+
                '        <FermionAction>\n'+
                '         <FermAct>WILSON</FermAct>\n'+
                '         <Mass>{0}</Mass>\n'.format(m)+
                '         <AnisoParam>\n'+
                '           <anisoP>true</anisoP>\n'+
                '           <t_dir>3</t_dir>\n'+
                '           <xi_0>2.0</xi_0>\n'+
                '           <nu>2.0</nu>\n'+
                '         </AnisoParam>\n'+
                '         <FermionBC>\n'+
                '           <FermBC>SIMPLE_FERMBC</FermBC>\n'+
                '           <boundary>1 1 1 -1</boundary>\n'+
                '         </FermionBC>\n'+
                '        </FermionAction>\n'+
                '    <InvertParam>\n'+
                '      <invType>CG_INVERTER</invType>\n'+
                '      <RsdCG>1.0e-10</RsdCG>\n'+
                '      <MaxCG>1000</MaxCG>\n'+
                '    </InvertParam>\n'+
                '      </Param>\n'+
                '       <NamedObject>\n'+
                '        <gauge_id>default_gauge_field</gauge_id>\n'+
                '        <source_id>sh_{0}_sh_{1}_seqsource</source_id>\n'.format(x,y)+
                '        <prop_id>sh_{0}_sh_{1}_seqprop</prop_id>\n'.format(x,y)+
                '       </NamedObject>\n'+
                '    </elem>\n\n')
        f.write('   <elem>\n'+
                '      <Name>ERASE_NAMED_OBJECT</Name>\n'+
                '      <Frequency>1</Frequency>\n'+
                '      <NamedObject>\n'+
                '        <object_id>sh_{0}_sh_{1}_seqsource</object_id>\n'.format(x,y)+
                '      </NamedObject>\n'+
                '    </elem>\n\n')
        f.write(
                '    <elem>\n'+
                '      <Name>BAR3PTFN</Name>\n'+
                '      <Frequency>1</Frequency>\n'+
                '      <Param>\n'+
                '        <version>7</version>\n'+
                '        <j_decay>3</j_decay>\n'+
                '        <mom2_max>1</mom2_max>\n'+
                '      </Param>\n'+
                '      <NamedObject>\n'+
                '        <gauge_id>default_gauge_field</gauge_id>\n'+
                '        <prop_id>sh_{0}_prop</prop_id>\n'.format(x)+
                '        <bar3ptfn_file>bar3ptfn_sh_{0}_sh_{1}_CONFIGNAME_t{2}.dat</bar3ptfn_file>\n'.format(x,y,t_sink)+
                '        <seqprops>\n'+
                '          <elem>\n'+
                '            <seqprop_id>sh_{0}_sh_{1}_seqprop</seqprop_id>\n'.format(x,y)+
                '            <gamma_insertion>{0}</gamma_insertion>\n'.format(i)+
                '          </elem>\n'+
                '       </seqprops>\n'+
                '      </NamedObject>\n'+
                '    </elem>\n\n')
        f.write('   <elem>\n'+
                '      <Name>ERASE_NAMED_OBJECT</Name>\n'+
                '      <Frequency>1</Frequency>\n'+
                '      <NamedObject>\n'+
                '        <object_id>sh_{0}_sh_{1}_seqprop</object_id>\n'.format(x,y)+
                '      </NamedObject>\n'+
                '    </elem>\n\n')
    f.write('   <elem>\n'+
            '      <Name>ERASE_NAMED_OBJECT</Name>\n'+
            '      <Frequency>1</Frequency>\n'+
            '      <NamedObject>\n'+
            '        <object_id>sh_{0}_prop</object_id>\n'.format(x)+
            '      </NamedObject>\n'+
            '    </elem>\n\n')
    if(x == 2 or x == 4):
        f.write('   <elem>\n'+
                '      <Name>ERASE_NAMED_OBJECT</Name>\n'+
                '      <Frequency>1</Frequency>\n'+
                '      <NamedObject>\n'+
                '        <object_id>sh_{0}_prop_nodisp</object_id>\n'.format(x)+
                '      </NamedObject>\n'+
                '    </elem>\n\n')

f.write(
        '  </InlineMeasurements>\n'+
        '   <nrow>{0} {0} {0} {1}</nrow>\n'.format(s_size, t_size)+
        '</Param>\n'+
        '<RNG>\n'+
        '  <Seed>\n'+
        '    <elem>1</elem>\n'+
        '    <elem>2</elem>\n'+
        '    <elem>3</elem>\n'+
        '    <elem>4</elem>\n'+
        '  </Seed>\n'+
        '</RNG>\n'+
        '<Cfg>\n'+
        '  <cfg_type>SZINQIO</cfg_type>\n'+
        '  <cfg_file>CONFIGNAME.lime</cfg_file>\n'+
        '</Cfg>\n'+
        '</chroma>\n')
